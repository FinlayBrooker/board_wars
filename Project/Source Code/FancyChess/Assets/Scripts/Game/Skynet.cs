...
public class Skynet : AI {

    public void alterModifiers()
    {
        // alter evaluation modifiers
        if (GameLogic.instance.turn < 5)
        {
            moveMod = 5;
        }
        else
        ...
        if (GameLogic.instance.one.isWhite == isWhite)
        {
            mypoints = GameLogic.instance.one.points;
            enemyPoints = GameLogic.instance.two.points;
        }
        else
        {
            mypoints = GameLogic.instance.two.points;
            enemyPoints = GameLogic.instance.one.points;
        }
    ...
    public int checkMoves(List<List<Tile>> checkboard, bool checkisWhite)
    {
        alterModifiers();
        ...
        // get current board value
        //should deep copy the board
        foreach (List<Tile> item in checkboard)
        {
            List<Tile> i = new List<Tile>();
            foreach (Tile t in item)
            {
        ...
        foreach (List<Tile> item in board)
        {
            foreach (Tile i in item)
            {
                if (i.occupied)
                {
                    if (i.occupiedBy.isWhite == checkisWhite)
                    {
                        myPieces.Add(new Piece(i.occupiedBy));
                    }
                    else
